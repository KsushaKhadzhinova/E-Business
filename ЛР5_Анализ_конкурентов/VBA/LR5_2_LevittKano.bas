Attribute VB_Name = "LR5_2_LevittKano"
Option Explicit
' =====================================================================================
' ЛР5 «Анализ конкурентов» — NotaCode. Модуль 2: цифровой товар (Левитт, Кано)
' Шаблон: Материалы\Шаблон_анализа_конкурентов_Левитт_Кано.xlsx
'
' КАК ЗАПУСТИТЬ
'   1. Скопируйте шаблон в папку ЛР5_Анализ_конкурентов и сохраните копию как .xlsm
'      (например, ЛР5_2_Левитт_Кано_NotaCode.xlsm). Рядом должна лежать папка VBA\data (TSV-файлы).
'   2. Alt+F11 -> File -> Import File... -> VBA\LR5_2_LevittKano.bas
'   3. Alt+F8 -> LR5_2_LevittKano.Main -> Выполнить.
...
' файлов VBA\data\l_*.tsv (их создаёт Python\export_vba_data.py из Python\lr5_data.py).
' Если комментарии в редакторе отображаются «кракозябрами» — это косметика, код работает.
'
' ЧТО ДЕЛАЕТ МАКРОС
'   01_Конкуренты (12), 02_Карта_сайтов (34 стр.), 03_Левитт (57 признаков), 04_Кано (29 требований),
'   05_Матрица_товара, 06_Сравнение (P01-P25 x C01-C08), 07_Стандарт_рынка (19), 08_Преимущества (11),
'   09_Дашборд B21:B25; ИСПРАВЛЯЕТ 06_Сравнение F90:L91 (ссылка на E у всех); диаграмма баллов.
...
Public Sub Main()
    Dim wb As Workbook, p As String, ws As Worksheet, f As Variant
    Set wb = ActiveWorkbook
    p = DataDir(wb, "l_01_konkurenty.tsv")
    If p = "" Then MsgBox "LR5: data folder VBA\data not found (l_01_konkurenty.tsv)": Exit Sub
    LoadStrings p & "l_strings.tsv"
    Application.ScreenUpdating = False
    Application.Calculation = -4135
    For Each f In Array("l_01_konkurenty", "l_02_karta", "l_03_levitt", "l_04_kano", "l_05_matrica", _
                        "l_06_sravnenie", "l_06_fix", "l_07_standart", "l_08_preim", "l_09_dash", "l_vvod")
        LoadBlock wb, p & f & ".tsv"
    Next f
    ' l_06_fix: в шаблоне формулы F90:L91 ссылались на столбец E (у всех конкурентов балл C01) — исправлено
    Application.Calculation = -4105
    Application.CalculateFull
    Set ws = SheetByPrefix(wb, "06_")
    BarChart ws, "NotaCode_Scores", ws.Range("E6:L6"), ws.Range("E90:L90"), S("chart_title"), "Q6", False
